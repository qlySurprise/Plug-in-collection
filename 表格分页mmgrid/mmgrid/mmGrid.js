    ...
    MMGrid.prototype = {
        ...
        , _initEvents: function(){
            ...
            $body.on('click','td',function(e){
                ...
                // if (!$(this).children().children().hasClass("btn")){
            ...
            $body.on('click','tr > td .mmg-check',function(e){
                e.stopPropagation();
                var $this = $(this);
                if (that.rows()[$(this).parent().parent().parent().index()].id == '999999'){
            ...
            if(opts.checkCol){
                $head.find('th .checkAll').on('click', function(){
                    ...
                    if (that.rows()[that.rowsLength()-1].id == '999999'){
                        that.deselect(that.rowsLength()-1);
                    }
                    if(tabGrid.selectedRows().length!=0){
                        $("#checkeds").html("已选中"+tabGrid.selectedRows().length+"项");
                    }else {
                        $("#checkeds").html("");
                    }
                });
        ...
        , _populate: function(items){
            ...
            this._hideMessage();
            if(items && items.length !== 0 && (total==undefined || total>0)  && opts.cols){
                var tbodyHtmls = [];
        ...
        ,reload: function () {
            for (var i = 0; i < this.opts.plugins.length; i++) {
                var plugin = this.opts.plugins[i];
                var pageParamName = plugin.opts['pageParamName'];
                if (pageParamName) {
            ...
            for (var colIndex = 0; colIndex < $titles.length; colIndex++) {
                var status = $.data($titles[colIndex], 'sortStatus');
                if (status) {
                    $($titles[colIndex]).removeData("sortStatus");
                }
            }
            var $ths = this._expandThs();
            $ths.find('.mmg-sort').removeClass('mmg-asc').removeClass('mmg-desc');
